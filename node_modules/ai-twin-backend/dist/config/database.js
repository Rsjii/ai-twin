...
exports.userQueries = {
    create: async (email, handle, passwordHash, referralCode) => {
        const id = idGenerator_1.generateId.user();
        const now = new Date();
    ...
    findByEmail: async (email) => {
        const result = await db_1.db.query('SELECT id, email, "passwordHash", handle, name, dob, phone, bio, active, "referralCode", "createdAt", "profileImage" FROM "User" WHERE email = $1', [email]);
        return result.rows[0];
    },
    findById: async (id) => {
        const result = await db_1.db.query('SELECT id, email, "passwordHash", handle, name, dob, phone, bio, active, "referralCode", "createdAt", "profileImage" FROM "User" WHERE id = $1', [id]);
        return result.rows[0];
    },
    findByReferralCode: async (referralCode) => {
        const result = await db_1.db.query('SELECT id, email, "passwordHash", handle, name, dob, phone, bio, active, "referralCode", "createdAt", "profileImage" FROM "User" WHERE "referralCode" = $1', [referralCode]);
        return result.rows[0];
    },
...
exports.twinQueries = {
    create: async (userId, styleVector, sampleReply, instructions) => {
        const id = idGenerator_1.generateId.twin();
    ...
    findByUserId: async (userId) => {
        const result = await db_1.db.query('SELECT id, "userId", "styleVector", "sampleReply", "isPublic", "publicHandle", "bio", "profileImage", "verified", "likeCount", "followCount", "chatCount", "createdAt" FROM "Twin" WHERE "userId" = $1', [userId]);
        return result.rows;
    },
    ...
    findById: async (twinId) => {
        const result = await db_1.db.query('SELECT id, "userId", "styleVector", "sampleReply", "instructions", "isPublic", "publicHandle", "bio", "profileImage", "verified", "likeCount", "followCount", "chatCount", "createdAt" FROM "Twin" WHERE id = $1', [twinId]);
        return result.rows[0];
    },
    delete: async (twinId, userId) => {
        await (0, twinUtils_1.verifyTwinOwnership)(twinId, userId);
        const result = await db_1.db.query('DELETE FROM "Twin" WHERE id = $1 AND "userId" = $2 RETURNING *', [twinId, userId]);
        return result.rows[0];
    }
};
exports.chatQueries = {
    create: async (userId, twinId) => {
        const id = idGenerator_1.generateId.chat();
        const result = await db_1.db.query('INSERT INTO "Chat" (id, "userId", "twinId") VALUES ($1, $2, $3) RETURNING *', [id, userId, twinId]);
        return result.rows[0];
    },
    findByUserId: async (userId) => {
        const result = await db_1.db.query('SELECT id, "userId", "twinId", "createdAt" FROM "Chat" WHERE "userId" = $1', [userId]);
        return result.rows;
    }
};
exports.messageQueries = {
    create: async (chatId, sender, content, approved = false) => {
        const id = idGenerator_1.generateId.message();
    ...
    findByChatId: async (chatId) => {
        const result = await db_1.db.query('SELECT id, "chatId", sender, content, approved, "createdAt" FROM "Message" WHERE "chatId" = $1 AND approved = true ORDER BY "createdAt" ASC', [chatId]);
        return result.rows;
    }
};
exports.otpQueries = {
    create: async (email, codeHash, expiresAt) => {
        const id = idGenerator_1.generateId.otp();
    ...
    findByEmail: async (email) => {
        const result = await db_1.db.query('SELECT id, email, "codeHash", "expiresAt", "createdAt", used FROM "OTP" WHERE email = $1 ORDER BY "createdAt" DESC LIMIT 1', [email]);
        return result.rows[0];
    },
    markAsUsed: async (id) => {
...
exports.publicTwinQueries = {
    ...
    findByPublicHandle: async (publicHandle) => {
       ...
       WHERE t."publicHandle" = $1 
         AND t."isPublic" = true
         AND (t."blockNonLoggedUsers" = false OR t."blockNonLoggedUsers" IS NULL)`, [publicHandle]);
        return result.rows[0];
    },
...
exports.twinLikeQueries = {
    create: async (twinId, userId) => {
        const id = idGenerator_1.generateId.like();
...
exports.twinFollowQueries = {
    create: async (twinId, userId) => {
        const id = idGenerator_1.generateId.follow();
...
exports.publicChatQueries = {
    create: async (twinId, visitorId, userId) => {
        ...
        if (!userId && visitorId) {
            await exports.publicChatQueries.cleanupOldAnonymousChats(twinId, 100);
        }
    ...
    cleanupOldAnonymousChats: async (twinId, keepCount = 100) => {
        try {
            const keepResult = await db_1.db.query(`
        SELECT id
        FROM "PublicChat"
        WHERE "twinId" = $1 
          AND "userId" IS NULL 
      ...
      `, [twinId, keepCount]);
            ...
            const deleteResult = await db_1.db.query(`
      ...
      `, [twinId, ...keepIds]);
            ...
            if (deletedCount > 0) {
                logger_1.logger.info(`[cleanupOldAnonymousChats] Cleaned up ${deletedCount} old anonymous chats for twin ${twinId}`);
                await db_1.db.query(`
          UPDATE "Twin" 
        ...
        `, [deletedCount, twinId]);
            ...
            logger_1.logger.error('Error cleaning up old anonymous chats:', error);
        }
    },
    updateMessageCount: async (chatId) => {
        const utcTimestamp = new Date().toISOString();
        const result = await db_1.db.query('UPDATE "PublicChat" SET "messageCount" = "messageCount" + 1, "lastActivity" = $2::timestamptz WHERE id = $1 RETURNING *', [chatId, utcTimestamp]);
        return result.rows[0];
    },
...
exports.styleAnchorsQueries = {
    create: async (twinId, userUtterance, idealReply, tags = [], type = 'interaction', phrase, patternType, context) => {
        const id = idGenerator_1.generateId.anchor();
...
exports.styleCorrectionsQueries = {
    create: async (twinId, knob, delta, source) => {
        const id = idGenerator_1.generateId.correction();
...
exports.aiRunsQueries = {
    create: async (twinId, mode, tokensIn, tokensOut, latencyMs, criticScore, regen = false) => {
        const id = idGenerator_1.generateId.run();
        const result = await db_1.db.query('INSERT INTO "ai_runs" (id, twin_id, mode, tokens_in, tokens_out, critic_score, regen, latency_ms) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, twin_id, mode, tokens_in, tokens_out, critic_score, regen, latency_ms, ts', [id, twinId, mode, tokensIn, tokensOut, criticScore || null, regen, latencyMs]);
        return result.rows[0];
    },
    findByTwinId: async (twinId, limit = 100, offset = 0) => {
        const result = await db_1.db.query('SELECT id, twin_id, mode, tokens_in, tokens_out, critic_score, regen, latency_ms, ts FROM "ai_runs" WHERE twin_id = $1 ORDER BY ts DESC LIMIT $2 OFFSET $3', [twinId, limit, offset]);
        return result.rows;
    },
    getQualityMetrics: async (twinId, days = 7) => {
       ...
       FROM "ai_runs" 
       WHERE twin_id = $1 AND ts >= NOW() - INTERVAL $2`, [twinId, `${days} days`]);
        return result.rows[0];
    },
    getRecentRuns: async (twinId, hours = 24) => {
        const result = await db_1.db.query('SELECT id, twin_id, mode, tokens_in, tokens_out, critic_score, regen, latency_ms, ts FROM "ai_runs" WHERE twin_id = $1 AND ts >= NOW() - INTERVAL $2 ORDER BY ts DESC', [twinId, `${hours} hours`]);
        return result.rows;
    }
};
exports.memorySessionQueries = {
    create: async (chatId, summary, keyTopics, vector) => {
        const id = idGenerator_1.generateId.memSess();
        const utcTimestamp = new Date().toISOString();
       ...
       RETURNING *`, [id, chatId, summary, keyTopics, JSON.stringify(vector), 0, utcTimestamp]);
        return result.rows[0];
    },
    findByChatId: async (chatId) => {
    ...
    update: async (chatId, summary, keyTopics, vector, messageCount) => {
       ...
       WHERE "chatId" = $6
       RETURNING *`, [summary, keyTopics, JSON.stringify(vector), messageCount, utcTimestamp, chatId]);
        return result.rows[0];
    }
};
exports.memoryLongTermQueries = {
    create: async (twinId, key, value, category, source = 'session') => {
        const id = idGenerator_1.generateId.memLt();
        const utcTimestamp = new Date().toISOString();
       ...
       RETURNING *`, [id, twinId, key, value, category, source, utcTimestamp]);
        return result.rows[0];
    },
