...
class MemoryService {
    async createOrUpdateSessionMemory(chatId, messages) {
        try {
           ...
           WHERE "chatId" = $5`, [summary, keyTopics, JSON.stringify(sessionVector), messages.length, utcTimestamp, chatId]);
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7::timestamptz)`, [id, chatId, summary, keyTopics, JSON.stringify(sessionVector), messages.length, utcTimestamp]);
    ...
    async storeLongTermMemory(twinId, key, value, category = 'fact', source = 'session') {
        try {
         ...
         DO UPDATE SET value = EXCLUDED.value, category = EXCLUDED.category, "updatedAt" = $7::timestamptz)`, [id, twinId, key, value, category, source, utcTimestamp]);
