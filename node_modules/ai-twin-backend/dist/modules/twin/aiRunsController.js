...
const createRun = async (req, res) => {
    try {
        ...
        const twin = await database_1.db.query('SELECT * FROM "Twin" WHERE id = $1 AND "userId" = $2', [twinId, userId]);
...
const updateRun = async (req, res) => {
    try {
        ...
        const result = await database_1.db.query(`UPDATE ai_runs SET ${updateFields.join(', ')} WHERE id = $${paramCount++} AND twin_id = $${paramCount++} RETURNING *`, values);
