...
exports.getChatSummary = exports.generateChatTitle = exports.updateChatTitle = exports.createNewChat = exports.deleteChat = exports.handleUserMessage = exports.sendMessage = exports.generateDraft = exports.continueChat = exports.getChatMessages = exports.getChatHistory = exports.getUserChats = exports.getChat = exports.startChat = void 0;
...
const startChat = async (req, res, next) => {
    ...
    `, [chatId, req.user.id, twin.id, utcTimestamp]);
        ...
        await (0, eventLogger_1.logEvent)(req.user.id, 'chat_started', { chatId: chat.id, twinId: twin.id });
        res.json({
            success: true,
            chatId: chat.id,
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to start chat');
    }
};
...
const getChat = async (req, res, next) => {
    ...
    `, [id]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to get chat');
    }
};
...
const getUserChats = async (req, res, next) => {
    ...
    `, [req.user.id]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to get chats');
    }
};
...
const getChatHistory = async (req, res, next) => {
    try {
        try {
            logger_1.logger.info('[getChatHistory:START]', {
                path: req.path,
                method: req.method,
                query: req.query,
            ...
            logger_1.logger.warn('[getChatHistory] Failed to log START context:', logError);
        }
        if (!req.user) {
            throw errors_1.createError.unauthorized();
        }
    ...
    `, [userId]);
        const chats = chatsResult.rows.map(chat => ({
            ...
            lastMessage: chat.lastMessage ? {
                ...
                relativeTime: chat.last_message_time ? (0, timestampUtils_1.formatRelativeTime)(chat.last_message_time) : null
            } : null,
            messageCount: chat.messageCount || 0,
            createdAt: (0, timestampUtils_1.normalizeTimestamp)(chat.createdAt),
            updatedAt: (0, timestampUtils_1.normalizeTimestamp)(chat.updatedAt),
        ...
        try {
            logger_1.logger.info('[getChatHistory:RESPONSE]', {
                userId,
            ...
            logger_1.logger.warn('[getChatHistory] Failed to log RESPONSE context:', logError);
        }
...
const getChatMessages = async (req, res, next) => {
    ...
    `, [id]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to get chat messages');
    }
};
...
const continueChat = async (req, res, next) => {
    ...
    `, [req.user.id, twin.id]);
      ...
      `, [
        ...
        await (0, eventLogger_1.logEvent)(req.user.id, existingChat ? 'chat_continued' : 'chat_started', { chatId: chat.id, twinId: twin.id });
        res.json({
            success: true,
            chatId: chat.id,
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to continue chat');
    }
};
...
const generateDraft = async (req, res, next) => {
    ...
    `, [chat.id]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to generate draft');
    }
};
...
const sendMessage = async (req, res, next) => {
    ...
    `, [
            idGenerator_1.generateId.message(),
            chat.id,
            'twin',
            content,
            true,
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to send message');
    }
};
...
const handleUserMessage = async (req, res, next) => {
    ...
    `, [id, req.user.id]);
      ...
      `, [id, req.user.id, twin.id, utcTimestamp]);
            ...
            try {
                await (0, eventLogger_1.logEvent)(req.user.id, 'chat_started', { chatId: newChat.id, twinId: twin.id });
        ...
        (async () => {
            try {
                ...
                await Promise.all([
                    ...
                    await (0, eventLogger_1.logEvent)(userId, 'chat_message', { chatId: chat.id, twinId: chat.twinId, userMessageId: userMessage.id, aiMessageId: aiMessage.id }),
                ...
                if (shouldExtractFacts && chat.twinId) {
                    ...
                    if (sessionMem?.summary) {
                        ...
                        memoryService.extractLongTermFacts(chat.twinId, contextText)
                            .then(() => {
                            logger_1.logger.info(`✅ Facts extracted from recent context for twin ${chat.twinId}`);
                        })
                            .catch(err => logger_1.logger.error('Fact extraction failed:', err));
                    }
                }
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to handle user message');
    }
};
...
async function updateStyleVectorAfterChat(twinId, userId) {
    ...
    `, [twinId, userId]);
      ...
      LIMIT ${constants_1.QUERY_LIMITS.RECENT_MESSAGES}
    `, [twinId, userId]);
...
const deleteChat = async (req, res, next) => {
    ...
    `, [id]);
        try {
            await (0, eventLogger_1.logEvent)(req.user.id, 'chat_deleted', { chatId: id });
        }
        catch (error) {
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to delete chat');
    }
};
...
const createNewChat = async (req, res, next) => {
    ...
    `, [chatId, userId, twinId, 'New Chat', 0, utcTimestamp]);
        const chat = chatResult.rows[0];
        await (0, eventLogger_1.logEvent)(userId, 'chat_created', { chatId: chat.id, twinId: chat.twinId });
        res.json({
            success: true,
            chat: {
                id: chat.id,
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to create new chat');
    }
};
...
const updateChatTitle = async (req, res, next) => {
    ...
    `, [title, utcTimestamp, chatId]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to update chat title');
    }
};
...
const generateChatTitle = async (req, res, next) => {
    ...
    `, [title, utcTimestamp, chatId]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to generate chat title');
    }
};
...
const getChatSummary = async (req, res, next) => {
    ...
    `, [chatId, userId]);
        ...
        (0, errorHandler_1.handleControllerError)(error, 'Failed to get chat summary');
    }
};
exports.getChatSummary = getChatSummary;
async function generateTitleFromMessage(message) {
    try {
        const { llmClient } = await Promise.resolve().then(() => __importStar(require('../../services/llmClient')));
...
//# sourceMappingURL=privateChatController.js.map