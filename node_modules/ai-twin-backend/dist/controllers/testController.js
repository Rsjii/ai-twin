...
const testDatabase = async (_req, res) => {
    try {
        ...
        const appError = errors_1.createError.internal('Database error', error);
...
const testAuth = async (req, res) => {
    try {
        ...
        const appError = errors_1.createError.internal('Auth error', error);
        return res.status(appError.statusCode).json({
            error: appError.message,
            errorCode: appError.errorCode
        });
...
const testOTP = async (req, res) => {
    try {
        ...
        const appError = errors_1.createError.internal('OTP operation error', error);
        return res.status(appError.statusCode).json({
            error: appError.message,
            errorCode: appError.errorCode
        });
...
const testProfile = async (req, res) => {
    ...
    try {
        ...
        const appError = errors_1.createError.internal('Internal server error', error);
        return res.status(appError.statusCode).json({
            error: appError.message,
            errorCode: appError.errorCode
        });
