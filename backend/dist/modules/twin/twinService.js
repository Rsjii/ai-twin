...
Return only valid JSON, no other text.`;
            const llmResponse = await llmClient_1.llmClient.generateResponse([
...
Please respond in JSON format with "response" and "title" fields. Title should be max 30 characters.`;
            ...
            const llmResponse = await Promise.race([
                llmClient_1.llmClient.generateResponse([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('LLM API timeout')), 30000))
            ]);
            ...
            if (isFirstMessage) {
                try {
                    ...
                    logger_1.logger.error('[TWIN SERVICE] JSON parse error for first message:', e instanceof Error ? e.message : String(e));
                    if (result && result.trim().length > 0) {
            ...
            if (context.twinId) {
                try {
                    ...
                    await db.query('INSERT INTO ai_runs (id, twin_id, mode, tokens_in, tokens_out, latency_ms) VALUES ($1, $2, $3, $4, $5, $6)', [
                        ...
                        context.twinId,
                        'human',
                        llmResponse.tokensUsed ? Math.floor(llmResponse.tokensUsed * 0.7) : 0,
                        llmResponse.tokensUsed ? Math.floor(llmResponse.tokensUsed * 0.3) : 0,
                        Date.now() - startTime
                    ]);
                }
                catch (error) {
            ...
            if (context.isFirstMessage) {
                const fallbackResponse = this.generateFallbackResponse(context.currentMessages?.[0] || 'Hello', context.personaData || {});
