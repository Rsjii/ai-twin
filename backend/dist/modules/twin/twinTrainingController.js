...
exports.getTrainingProgress = exports.convertToTraining = exports.getTrainingEffectiveness = exports.convertMessagesToTraining = exports.getChatMessages = exports.addManualTraining = void 0;
const database_1 = require("../../config/database");
const logger_1 = require("../../config/logger");
const addManualTraining = async (req, res) => {
    try {
    ...
    `, [anchorId, twinId, userMessage, idealReply, [trainingType || 'manual']]);
        ...
        logger_1.logger.error('Manual training API error:', error);
        res.status(500).json({ success: false, error: 'Internal server error' });
    }
};
exports.addManualTraining = addManualTraining;
...
const convertMessagesToTraining = async (req, res) => {
    try {
        const { id: twinId } = req.params;
    ...
    `, messageIds);
        ...
        for (const chatId in chatGroups) {
            ...
            for (let i = 0; i < chatMessages.length - 1; i++) {
                ...
                if (userMessage.sender === 'user' && aiMessage.sender === 'ai') {
                    await database_1.styleAnchorsQueries.create({
                        twinId: twinId,
        ...
        logger_1.logger.error('Error converting messages to training:', error);
        res.status(500).json({ success: false, error: 'Failed to convert messages to training' });
    }
};
exports.convertMessagesToTraining = convertMessagesToTraining;
const getTrainingEffectiveness = async (req, res) => {
    try {
        const { id: twinId } = req.params;
        const userId = req.user.id;
        const twinResult = await database_1.db.query(`
    ...
    `, [twinId]);
        ...
        if (totalAnchors >= 10)
            ...
            score += (totalAnchors / 10) * 30;
        if (totalMemories >= 20)
            score += 30;
        else
            score += (totalMemories / 20) * 30;
        if (recentCorrections <= 5)
            score += 40;
        else
            score += Math.max(0, 40 - (recentCorrections - 5) * 5);
        ...
        logger_1.logger.error('Error calculating training effectiveness:', error);
        res.status(500).json({ success: false, error: 'Failed to calculate training effectiveness' });
    }
};
exports.getTrainingEffectiveness = getTrainingEffectiveness;
const convertToTraining = async (req, res) => {
    try {
        const { id: twinId } = req.params;
        const userId = req.user.id;
    ...
    `, [anchorId, twinId, messageResult.rows[0].content, idealReply, ['chat_conversion']]);
        ...
        logger_1.logger.error('Convert to training API error:', error);
        res.status(500).json({ success: false, error: 'Internal server error' });
    }
};
exports.convertToTraining = convertToTraining;
const getTrainingProgress = async (req, res) => {
    ...
    `, [twinId, userId]);
      ...
      WHERE twin_id = $1
    `, [twinId]);
        const recentResult = await database_1.db.query(`
    ...
    `, [twinId]);
        ...
        logger_1.logger.error('Training progress API error:', error);
        res.status(500).json({ success: false, error: 'Internal server error' });
    }
};
exports.getTrainingProgress = getTrainingProgress;
//# sourceMappingURL=twinTrainingController.js.map